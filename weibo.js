...
async function generateHotSearch(targets = 'all') {
  ...
  console.log('[weibo.js] 检查 getApiConfigForFeature:', {
    函数存在: !!window.getApiConfigForFeature,
    state存在: typeof state !== 'undefined',
    featureApis: window.state?.apiConfig?.featureApis?.weibo
  });
  const { proxyUrl, apiKey, model } = window.getApiConfigForFeature ? window.getApiConfigForFeature('weibo') : state.apiConfig;
  console.log('[weibo.js] 获取到的API配置:', { proxyUrl, apiKey: apiKey ? '***已设置***' : '未设置', model });
...
`;
  ...
  try {
    ...
    // 兼容处理：如果AI返回的是包装对象而非直接数组
    if (!Array.isArray(newComments) && typeof newComments === 'object') {
      newComments = newComments.data || newComments.comments || newComments.list || newComments.result || Object.values(newComments).find(v => Array.isArray(v));
    }
...
function getInitialWeiboStats(chat) {
  ...
  const keywords = ['AI工程师', '人工生命', '自由职业者', '歌手', '博主', '网红', '学生', '主播', '选手', '画家', '作家'];
...
{
  "fanName": "粉丝的微博昵称",
  "fanPersona": "对这位粉丝的简单描述 (例如: '一个熬夜的程序员')",
...
现在，请开始生成【只有粉丝发言】的私信列表。`;
  try {
    ...
    let newDmsData = JSON.parse(cleanedContent);
    // 兼容处理：如果AI返回的是包装对象而非直接数组
    if (!Array.isArray(newDmsData) && typeof newDmsData === 'object') {
      newDmsData = newDmsData.data || newDmsData.dms || newDmsData.messages || newDmsData.list || newDmsData.result || Object.values(newDmsData).find(v => Array.isArray(v));
    }
...
现在，请以JSON数组的格式，生成你接下来要发送的1-3条消息。`;
  ...
  try {
    ...
    let newMessages = JSON.parse(cleanedContent);
    // 兼容处理：如果AI返回的是包装对象而非直接数组
    if (!Array.isArray(newMessages) && typeof newMessages === 'object') {
      newMessages = newMessages.data || newMessages.messages || newMessages.list || newMessages.result || Object.values(newMessages).find(v => Array.isArray(v)) || [newMessages];
    }
