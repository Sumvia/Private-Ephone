document.addEventListener('DOMContentLoaded', () => {
  ...
  async function generateForumComments() {
    ...
    const { proxyUrl, apiKey, model } = window.getApiConfigForFeature ? window.getApiConfigForFeature('forum') : state.apiConfig;
...
`;
    ...
    try {
      ...
      let newCommentsData = JSON.parse(cleanedContent);
      // 兼容处理：如果AI返回的是包装对象而非直接数组
      if (!Array.isArray(newCommentsData) && typeof newCommentsData === 'object') {
        newCommentsData = newCommentsData.data || newCommentsData.comments || newCommentsData.list || newCommentsData.result || Object.values(newCommentsData).find(v => Array.isArray(v));
      }
...
`;
    ...
    try {
      ...
      const rawContent = isGemini ? data.candidates[0].content.parts[0].text : data.choices[0].message.content;
      const cleanedContent = rawContent.replace(/^```json\s*|```$/g, '').trim();
      let newPostsData = JSON.parse(cleanedContent);
      // 兼容处理：如果AI返回的是包装对象而非直接数组
      if (!Array.isArray(newPostsData) && typeof newPostsData === 'object') {
        newPostsData = newPostsData.data || newPostsData.posts || newPostsData.list || newPostsData.result || Object.values(newPostsData).find(v => Array.isArray(v));
      }
    ...
    try {
      ...
      try {
        ...
        if (!Array.isArray(parsedData) && typeof parsedData === 'object') {
          parsedData = parsedData.data || parsedData.stories || parsedData.list || parsedData.result || Object.values(parsedData).find(v => Array.isArray(v));
        }
        if (!Array.isArray(parsedData)) throw new Error('AI未返回数组格式。');
        stories = parsedData;
      } catch (e) {
