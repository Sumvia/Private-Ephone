...
async function refreshDatingScenes() {
  ...
  const { proxyUrl, apiKey, model } = window.getApiConfigForFeature ? window.getApiConfigForFeature('dating') : state.apiConfig;
  console.log('[date.js refreshDatingScenes] API配置:', {
    proxyUrl,
    apiKey: apiKey ? `***${apiKey.slice(-4)}` : '空',
    apiKeyLength: apiKey?.length,
    model,
    来源: window.getApiConfigForFeature ? 'getApiConfigForFeature' : 'state.apiConfig'
  });
  // 使用trim()确保空白字符串也被视为空
  if (!proxyUrl?.trim() || !apiKey?.trim() || !model?.trim()) {
  ...
  try {
    ...
    console.log('[date.js] AI原始返回内容:', cleanedContent.substring(0, 500));

    // 修复：如果AI返回的是多个对象连在一起（不是数组），尝试修复格式
    // 检测模式：以 { 开头但不是数组，且包含 },{ 或 }\n{ 模式
    if (cleanedContent.startsWith('{') && !cleanedContent.startsWith('[')) {
      // 尝试将多个对象包装成数组
      // 先尝试匹配 },{  或 }\n{  或 }\n\n{ 等模式，替换为 },{
      const fixedContent = '[' + cleanedContent.replace(/\}\s*,?\s*\{/g, '},{') + ']';
      try {
        const testParse = JSON.parse(fixedContent);
        if (Array.isArray(testParse) && testParse.length > 1) {
          console.log('[date.js] 检测到AI返回了多个连续对象，已修复为数组格式');
          cleanedContent = fixedContent;
        }
      } catch (e) {
        // 修复失败，保持原样
        console.log('[date.js] 尝试修复多对象格式失败，保持原样');
      }
    }

    let newScenes = JSON.parse(cleanedContent);
    console.log('[date.js] 解析后的数据类型:', typeof newScenes, Array.isArray(newScenes) ? '(是数组)' : '(不是数组)');
    console.log('[date.js] 解析后的数据:', newScenes);

    // 兼容处理：如果AI返回的是包装对象而不是直接的数组，尝试提取数组
    if (!Array.isArray(newScenes) && typeof newScenes === 'object' && newScenes !== null) {
      console.log('[date.js] 对象的所有键:', Object.keys(newScenes));
      // 尝试从常见的包装键中提取数组
      const possibleKeys = ['scenes', 'data', 'results', 'items', 'list', 'dating_scenes', 'datingScenes', 'dating', 'dates', 'options'];
      for (const key of possibleKeys) {
        if (Array.isArray(newScenes[key])) {
          console.log(`[date.js] AI返回了包装对象，从 "${key}" 键提取数组`);
          newScenes = newScenes[key];
          break;
        }
      }
      // 如果还不是数组，尝试获取对象的第一个数组值
      if (!Array.isArray(newScenes)) {
        const firstArrayValue = Object.values(newScenes).find(v => Array.isArray(v));
        if (firstArrayValue) {
          console.log('[date.js] AI返回了包装对象，提取第一个数组值');
          newScenes = firstArrayValue;
        } else {
          // 最后的兜底：如果对象本身看起来像一个有效的场景对象，将其包装成数组
          if (newScenes.name && (newScenes.cost !== undefined || newScenes.imagePrompt)) {
            console.log('[date.js] AI返回了单个场景对象，将其包装成数组');
            newScenes = [newScenes];
          } else {
            console.log('[date.js] 对象中没有找到任何数组值，且不是有效的场景对象');
          }
        }
      }
    }
